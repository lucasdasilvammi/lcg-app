...
export default function DuelRules({ roomData, currentUserId, acknowledgeRules }) {
 ...
 <h1 className="text-5xl font-hakobi uppercase text-light">quelques règles</h1>
            <div className="flex min-h-0 w-full flex-1 flex-col gap-3 overflow-y-auto pr-3 text-center">
              {content.rules.map((rule, index) => (
 <p key={index} className="font-funnel text-base leading-snug text-light opacity-80">{rule}</p>
              ))}
              {content.hint && (
                <div className="flex items-center mt-auto gap-3 text-left opacity-70">
 ...
 <div className='flex w-full flex-col gap-4'>
            <div className="flex w-full flex-col items-center gap-3">
              {readerPlayer && type !== 'pick' && type !== 'zoom' && <CharacterTag charId={readerPlayer.character} className="mt-0 mb-0 self-center" />}
