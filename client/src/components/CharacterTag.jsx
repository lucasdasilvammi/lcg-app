...
export default function CharacterTag({ charId, text, className = '', hideName = false, showAvatar = !hideName, icon = null, reversed = false, nameOnly = false, variant = 'default' }) {
  ...
  return (
    <div
      className={`relative inline-flex items-center ${isMini ? 'h-8 gap-1.5 px-2 py-1' : 'h-10 gap-3 px-3 py-2'} ${className}`}
    ...
    >
      ...
      <div className="z-10 flex items-center gap-1">
        ...
        <span className={`shrink-0 whitespace-nowrap font-funnel font-semibold ${isMini ? 'text-base' : 'text-lg'}`}>{tagText}</span>
      </div>
